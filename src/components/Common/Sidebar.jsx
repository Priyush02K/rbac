...
const Sidebar = () => {
  ...
  return (
    <div className="relative">
      <aside
        className={`fixed top-0 left-0 h-screen bg-[#4B0082] text-white z-40 sm:w-64 w-60 shadow-lg transition-transform duration-300 ease-in-out ${
          isSidebarOpen ? "translate-x-0" : "-translate-x-full"
        } sm:translate-x-0`}
      >
        <div className="w-full flex justify-center py-7 mb-8 border-b border-gray-700">
        <AiOutlineChrome className="text-2xl mb-4	 font-bold	" />
        ...
        <ul className="flex flex-col gap-4 w-full px-4">
          {menuItems.map((menu, index) => (
            <li key={index} className="w-full">
              <Link
                to={menu.path}
                className={`flex items-center gap-4 w-full h-12 px-4 rounded-md text-left text-white/90 hover:bg-black font-bold	  transition-all ${
